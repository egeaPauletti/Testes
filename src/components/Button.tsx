...
const Button: React.FC<BtnProps> = ({ filled = false, label, onClick }) => {
  return (
    <div>
      <button
        className={`relative cursor-pointer 2xl:py-2 2xl:px-9 text-center font-barlow inline-flex justify-center 2xl:text-base text-sm px-6 py-1.5 text-white rounded-lg border-solid transition-transform duration-300 ease-in-out group outline-offset-4  focus:outline-[#2ea98c] focus:outline-offset-4 overflow-hidden hover:opacity-75 ${
          filled ? "bg-[#2ea98c]" : ""
        }`}
        onClick={onClick}
      >
